...
> 当前项目进度总览模板。完整 Gate/Checkpoint 表格统一保存在 `gates.md`；本文件只引用最近 `gate_id` 和结论。
...
| 执行模式 | 单 MR 执行 / 连续项目执行 |
| MR 启动人工确认 | 需要 / 不需要 |
| stage_epoch | int（与 coder-current-task.md / gates.md 必须相等） |
...
| 跨 IDE / Model 恢复入口 | `handoff.md` + `task-state.md` + 活动执行契约 |
...
| 三文件 stage_epoch 一致（coder-current-task / project-progress / gates） | PASS / FAIL |  |
