...
本文件只描述模块边界和维护依赖，不替代 `skills/*/SKILL.md` 的触发规则，也不复制各协议正文。执行任务时先进入 `skills/superCoder/SKILL.md`，再按路由读取具体子技能的 `references/` 或主技能公共资源。本索引中以 `skills/`、`harness/` 或 `tests/` 开头的路径均相对仓库根目录。
...
- 模块和文件映射由 `skills/superCoder/config/module-map.yaml` 记录，供维护校验使用。
...
- `skills/superCoder/config/module-map.yaml` 不承载自然语言执行规则，只记录可检查的文件映射。
- `skills/superCoder/assets/templates/` 不承载触发边界、任务是否可执行的判断规则。

## 修改同步检查

| 修改内容 | 必须同步检查 |
|---|---|
| 新增或移动协议文件 | `skills/`、本文件、`skills/superCoder/config/module-map.yaml` |
| 新增或调整 Skill Pack 子技能 | `skills/`、本文件、`skills/superCoder/config/module-map.yaml`、`tests/skill-behavior/` |
