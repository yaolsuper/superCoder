...
- 生成或修复 analysis / plan / fix MR 产物时读取 `../superCoder-planning/references/planning.md`
...
- 要求具备文件化的故障现象、复现证据、文件和行级根因、修复范围以及回归验证计划。
- 拒绝 `inline_hotfix_root_cause` 或 `inline_hotfix_single_slice` 等内联 hotfix 链路。
- 确保 BUG / hotfix MR 内容来自根因矩阵，而不是紧急程度或聊天记忆。
- 当证据链不完整时返回阻断结论。
