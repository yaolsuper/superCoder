...
按 `skills/superCoder-checkpoint/references/checkpoint.md` 的 `Checkpoint Review` 格式输出：

- Review Summary / Checkpoint Review（复核摘要 / Checkpoint 复核）
- Issues（问题清单）
- Scenario Checklist Result（场景清单结果）
- Traceability（可追溯性）
- Step Order Check（步骤顺序检查）
- Final Decision（最终决策）
```
