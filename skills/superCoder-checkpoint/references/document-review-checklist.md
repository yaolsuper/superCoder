...
本文件负责“场景质量标准”，`skills/superCoder-checkpoint/references/checkpoint.md` 负责“流程门禁”。正式 Review 必须同时说明：
...
| 场景 | 上游依据 | 下游目标 | 必用检查清单 |
|---|---|---|---|
| BRD | 原始需求、业务背景、用户输入 | PRD | BRD 检查清单（BRD Checklist） |
| PRD | BRD、用户场景、业务边界 | ADD / 验收设计 | PRD 检查清单（PRD Checklist） |
| ADD | PRD、技术约束、现有架构 | LLD | ADD 检查清单（ADD Checklist） |
| LLD | ADD、接口/模块边界 | DBD / MR | LLD 检查清单（LLD Checklist） |
| DBD | LLD 数据流、查询路径、迁移约束 | MR / migration | DBD 检查清单（DBD Checklist） |
| MR | PRD / ADD / LLD / DBD / 计划项 | 开发执行 | MR 检查清单（MR Checklist） |
| ANALYSIS | 用户目标、代码事实、命令证据 | PLAN | 分析检查清单（Analysis Checklist） |
| PLAN | 分析报告、约束、风险 | MR | 计划检查清单（Plan Checklist） |
| CURRENT_TASK | 分析、计划、MR | 当前执行 | 当前任务检查清单（Current Task Checklist） |
| EXECUTION_RECORD | 当前 MR、实际 diff、验证结果 | 验收决策 | 执行记录检查清单（Execution Record Checklist） |
| ACCEPTANCE_DECISION | 执行记录、验证、Checkpoint 状态 | 下一 MR / 交付 | 验收检查清单（Acceptance Checklist） |
...
| 语义维度完整 | 集合/数组、单复数文案、相似能力复用或并行聚合场景已分别核对类型、基数、空值、顺序、重复、运行态编码和合并/冲突规则；数据形状不替代业务语义 |
| 负向约束有来源 | “只能一个”“至少一个”“最多 N 个”“多值应失败”、截断、去重、覆盖和冲突失败均可追溯到用户输入、权威来源或人工 Decision |
...
### 执行记录检查清单（Execution Record Checklist）
...
在 `skills/superCoder-checkpoint/references/checkpoint.md` 的 Review 输出基础上，增加：
