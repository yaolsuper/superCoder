...
当用户要求“分析”“调研”“评估”“迁移方案”“开发实施计划”“MR 拆分”“阶段计划”时读取本文件。此类任务只生成执行产物，不修改产品代码；若用户明确要求开始编码，再切换到 `references/protocols/execution.md`。

需要项目进度总览卡片模板时，只读取 `assets/templates/progress-overview.md`。需要当前任务或 MR 文件结构时，只读取 `assets/templates/task-and-mr.md`。
需要对分析报告、实施计划、MR 文件、当前任务契约或最终交付做专项复核时，读取 `references/protocols/checkpoint.md`。
需要按 BRD、PRD、ADD、LLD、DBD、MR 或 Coder 产物类型做场景化 Review 时，读取 `references/document-review-checklist.md`。
...
BUG 修复、缺陷修复、线上问题修复、回归修复等任务**不享受任何链路豁免**，必须走与功能开发相同的完整链路：

```text
analysis(含根因证据矩阵) -> implementation plan(含根因映射) -> fix-mr(含根因修复范围) -> current task -> execution
```

不得因“只是一个 bug”“改动很小”“用户只说修一下”而跳过 analysis、plan 或 fix-mr 中任意一环。`source_chain.plan: null` 或 `source_chain.mr: null` 同样禁止进入编码；用空壳 plan / 空壳 fix-mr 文件糊弄（文件存在但内容不对应本次 bug）视为编造，CP2/CP4 必须 FAIL。

BUG 修复任务的 plan 必须以**根因证据矩阵**为强制内容，作为放行依据替代“用户对方案的确认”（用户确认的是“要不要修”，根因证据确认的是“怎么修才对”）。根因证据矩阵至少包含：
...
根因定位缺失、复现证据缺失、或修复范围无法回溯到根因定位的，plan 的 CP2 必须 FAIL，不得生成 fix-mr，不得进入执行。这条链比功能开发的“用户拍板”更可审计，不构成降级。

BUG 修复任务的 fix-mr 文件 `## 来源链路` 必须显式标注来源类型为根因证据矩阵的结论 ID（如 R1、R2），并说明修复范围如何对应根因定位。
...
`development_project_id` 的推导规则见 `references/glossary.md`。若无法从任务或配置推导，使用本次目标的短横线命名，例如 `python-migration`。
...
- 下一步只能是生成待确认实施计划，不能直接生成详细 MR 文件或进入 `references/protocols/execution.md`。
...
- 不把任何 MR 候选标记为 `READY`。
- 不进入 `references/protocols/execution.md`。
- 不触发 `stage_epoch` 跳变到 MR_SPLIT 或更后的阶段（见 `SKILL.md` 阶段转换原子性）。
- 最终回复必须说明计划处于待确认状态，并列出需要用户确认或调整的点。

计划确认是一次阶段转换，必须按 `SKILL.md` 阶段转换原子性执行：在写状态文件前先输出 `assets/templates/gates.md` 的“状态回写计划门禁”，把 PLANNING → MR_SPLIT 的 `stage_epoch` 三文件同步跳变记录为表格，触发词、前后阶段、epoch 变化必须可追溯。用户口头确认不得单独构成转换；含糊指令（“继续”“接着做”）在计划待确认时只能原地等待或请求澄清，不得升级，详见 `references/protocols/execution.md` 阶段升级裁决规则。
...
完成分析、规划或 MR 拆分后，回复中必须列出生成的规范产物路径。最终回复前必须确认这些路径已创建，并确认项目进度总览卡片、Checkpoint 状态和 handoff 已更新到本轮状态。若没有生成预期产物，必须说明缺失原因和下一步需要的输入，不能声明完成。
