...
当需要生成分析报告、实施计划、MR 文件、当前任务契约、执行记录、验收决策，或用户明确要求控制输出幻觉、目标偏移、步骤跳跃和依赖顺序时读取本文件。若需要按 BRD、PRD、ADD、LLD、DBD、MR 或 Coder 产物类型执行专项 Review，同时读取 `references/document-review-checklist.md`。若需要显式分离生成、审查、修复角色，分别读取 `references/prompts/generator.md`、`references/prompts/reviewer.md`、`references/prompts/fixer.md`。

Checkpoint 的目标不是限制模型提出方案，而是在每个正式产物进入下游前做硬复核，确保输出有依据、符合目标、遵守计划顺序，并且依赖未满足时不会继续扩散错误。

## 核心规则

- Generator、Reviewer、Fixer 必须分阶段执行：先生成，再复核，再按问题修复，再复核。不得在同一段输出中同时宣布生成、修复和放行；需要角色提示词时读取 `references/prompts/` 下对应文件。
...
- 通用复核清单不能替代场景 checklist。正式 Review 必须先判定 `document_type`，再使用 `references/document-review-checklist.md` 中对应 checklist。
...
| CP5 最终交付 | 最终回复、验收决策或进入开发执行前 | 确认文档链路、风险关闭、MR 依赖、验收方式、Start Gate、输出、代码、验证、记录、进度和 Checkpoint 状态一致 |
...
CP5 的目标是在最终回复、验收决策、进入开发执行或切换下一 MR 前做总放行。必须检查：
...
| Start Gate 满足 | 当前任务具备 source_chain、路径守卫、验证命令、Checkpoint 状态和允许动作 |
| pre-edit guard 满足 | 当前阶段为 RUNNING、三文件 stage_epoch 一致、source_chain.plan/mr 文件真实存在、路径守卫通过 |
...
| 状态一致 | `project-progress.md`、`coder-current-task.md`、`checkpoint-status.md`、`handoff.md`、执行记录、复核报告和最终回复不矛盾 |
...
| 风险 | 表现 | 处理规则 |
|---|---|---|
| Codex 自行脑补 | 新增未声明需求、架构、字段、接口、路径或验证结果 | 标记为【待确认】或删除；若影响下游，按 Blocker 处理 |
| 文档职责混淆 | PRD 写技术、ADD 写字段、LLD 改架构、DBD 新增业务概念 | 回到职责边界修复；对应场景 checklist 必须 FAIL |
| 下游断链 | PRD 找不到 BRD 依据、MR 找不到 PRD/LLD/DBD 依据 | CP3 FAIL，补 Traceability Matrix 后复审 |
