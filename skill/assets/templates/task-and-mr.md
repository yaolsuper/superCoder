...
本文件只在需要当前任务结构、MR 文件结构、状态维护或偏差记录模板时读取。门禁表格见 `assets/templates/gates.md`，项目进度总览卡片见 `assets/templates/progress-overview.md`。
...
状态名称和含义见 `references/glossary.md`。必须在当前任务文件中显式维护状态：
...
偏差类型和含义见 `references/glossary.md`。
